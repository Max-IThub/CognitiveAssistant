...
class ChatGPTTelegramBot:
    ...
    def __init__(self, config: dict, openai: OpenAIHelper):
        ...
        self.commands = [
            ...
            BotCommand(command='reset', description='Сбросить разговор. Опционально передавать высокоуровневые инструкции '
                                                    '(например, /reset Вы полезный помощник)'),
            BotCommand(command='image', description='Создать изображение из подсказки (например, /image cat)'),
            BotCommand(command='stats', description='Статистика использования'),
            BotCommand(command='resend', description='Повторить последнее сообщение')
        ]
        self.group_commands = [
            BotCommand(command='chat', description='Chat with the bot!')
        ] + self.commands
        self.disallowed_message = "Извините, вам не разрешено использовать этого бота. Для покупки доступка напишите владельцу @milk0vichmar "
    ...
    async def help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        help_text = 'Привет, я чат бот основанный на модели GPT-3.5 разработанный @milk0vichmar, напиши ему для покупки, а затем напиши мне любое сообщение и я помогу тебе' + \
                    '\n\n' + \
                    '\n'.join(commands_description) + \
                    '\n\n' + \
                    'Отправьте мне голосовое сообщение или файл, и я расшифрую его для вас!' + \
                    '\n\n' + \
                    "При поддержке https://t.me/milk0vichmar"
    ...
    async def prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        Реагируйте на входящие сообщения и отвечайте соответствующим образом.
        """
        if not await self.check_allowed_and_within_budget(update, context):
            return
        
        logging.info(f'Полученное новое сообщение от пользователя {update.message.from_user.name} (id: {update.message.from_user.id})')
        chat_id = update.effective_chat.id
        user_id = update.message.from_user.id
        ...
        if self.is_group_chat(update):
            ...
            if prompt.lower().startswith(trigger_keyword.lower()):
                ...
                if update.message.reply_to_message and update.message.reply_to_message.from_user.id == context.bot.id:
                    ...
                    logging.warning('Сообщение не начинается с ключевого слова триггера, игнорируя...')
                    return
